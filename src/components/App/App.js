...
const WHOLE_NUMBER_THRESHOLD = 15;
...
class App extends React.Component {
  ...
  setCurrentBracket(bracketNumber) {
    this.setState({ currentBracketNumber: bracketNumber, hasSetBracket: true }, () => {
      setTimeout(() => {
  ...
  setCurrentLga(lgaObject) {
    ...
    let percentageDifference = currentTopPercentValue.toFixed(1) - NATIONAL_AVERAGE_IN_TOP; // Aust-wide percent in top bracket
    ...
    const roundedPercentageDifference = Math.abs(percentageDifference.toFixed(1));
    const shouldBeWhole = currentTopPercentValue > WHOLE_NUMBER_THRESHOLD ? true : false;
    ...
    const stateText = `Zooming ${
      getStateInfo(stateCode).text !== 'Australian Capital Territory' ? 'out' : ''
    } to <strong>${getStateInfo(stateCode).text}</strong>, <strong>${parseFloat(
      getStateInfo(stateCode).percent
    ).toFixed(1)} per cent</strong> of income earners are in the top bracket, which is 
    ${
  ...
  render() {
    let top5 = getUserTopJobsData(topBracketData, this.state.currentBracketNumber);

    let user = getUserBracketData(userBracketData, this.state.currentBracketNumber);
